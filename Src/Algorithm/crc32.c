...
/**
 * @breif Reverse byte order of input 32bis word.
 * For example, input 0x12345678 will be reversed to 0x78563412
 * @param input a word need to be reserved.
 * @return Reversed word
 */
uint32_t ByteReverse(uint32_t input)
{
	uint32_t output;
	uint8_t* ptrOut = (uint8_t*)&output;
	uint8_t* ptrIn = (uint8_t*)&input;
...
uint32_t Crc32(uint32_t crc, const void* input, uint32_t len)
{
	...
	uint32_t len4 = len / 4;
	uint32_t moreLen = len % 4;
	while (len4--)
	{
		uint32_t data = ByteReverse(*pData++);
		uint8_t* ptr = (uint8_t*)&data;
		for (int i = 0; i < 4; ++i)
		{
			crc = (crc << 8) ^ crcTable[((crc >> 24) & 0xFF) ^ *ptr++];
		}
	}
	if (moreLen)
	{
		uint32_t remainData = 0;
		for (uint32_t pos = 0; pos < moreLen; pos++)
		{
			*(((uint8_t*)&remainData) + pos) = *((uint8_t*)pData + pos);
		}
		remainData = ByteReverse(remainData);
		uint8_t* ptr = (uint8_t*)&remainData;
		for (int i = 0; i < 4; ++i)
		{
			crc = (crc << 8) ^ crcTable[((crc >> 24) & 0xFF) ^ *ptr++];
		}
